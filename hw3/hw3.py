...
def quad_sim():
    ...
    # eul_angle[0] = rotation_error[0]
    # eul_angle[1] = rotation_error[1]
    # eul_angle[2] = rotation_error[2]
    ...
    while True:
        ...
        while t <= T:
            ...
            q.R = rotation_matrix(roll, pitch, yaw) #current R
            R_des_to_cur = np.matmul(des_R,q.R)
            V = 0.5*vee_map(R_des_to_cur - np.linalg.inv(R_des_to_cur))  #rotating axis 
            V_angle = acos((trace(R_des_to_cur)-1)/2)  #rotate angle
            rotation_error = V_angle*normalized(V)  #rotation error
