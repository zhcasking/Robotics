...
// turtle pose
turtlesim::Pose leader;       //pose contain coordinate of x y and theta
...
void Positioncontrol(geometry_msgs::Point &goal, turtlesim::Pose &follower, geometry_msgs::Twist &vel_msg) {
	...
	vel_msg.linear.x = error_norm;
   	vel_msg.angular.z = error_theta * 3; 

	if(error_norm < 0.1 )
		vel_msg.angular.z = 0;
}

void Positioncontrol_leader(geometry_msgs::Point &goal, turtlesim::Pose &turtle_pose, geometry_msgs::Twist &turtle_vel_msg)
{
	// error in inertia frame
	...
	turtle_vel_msg.linear.x = error_norm;
   	turtle_vel_msg.angular.z = 2 * error_theta; 
	   printf("%f\n",error_norm);

	if(error_norm < 0.0003 )
	{
...
int main(int argc, char **argv)
{
	...
	cin>>leader_goal.y;	
  	...
  	while (ros::ok()){
        ...
        leadertoworld2D( follower1_goal, leader); //get destination vector (follower1_goal.x and y)of world frame 
		leadertoworld2D( follower2_goal, leader);

		//Input your goal_point to your controller
		Positioncontrol_leader(leader_goal, leader, leader_twist);
